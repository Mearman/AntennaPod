...
public class GpodderSyncFlowsTest {
    ...
    public void testEpisodeActionFromServerMarksPlayedAndSetsPosition() throws Exception {
        ...
        assertTrue(SynchronizationSettings.isLastSyncSuccessful());
        await().atMost(30, TimeUnit.SECONDS).until(() -> anyUploadedActionIs(
                finishedItem.getMedia().getDownloadUrl(), "play"));
    }
    ...
    public void testMarkingEpisodePlayedInUiUploadsPlayAction() throws Exception {
        ...
        await().atMost(SYNC_WAIT_SECONDS, TimeUnit.SECONDS).until(() -> {
            for (JSONObject action : server.uploadedEpisodeActions) {
                if (markedItem.getMedia().getDownloadUrl().equals(action.optString("episode"))
    ...
    public void testForceFullSyncRestartsFromBeginning() throws Exception {
        ...
        await().atMost(SYNC_WAIT_SECONDS, TimeUnit.SECONDS).until(() -> {
            for (GpodderTestServer.RecordedRequest request : server.getRequests("GET", "/api/2/subscriptions/")) {
                if (request.path.contains("since=0")) {
                    return true;
                }
            }
            return false;
    ...
    private boolean anyUploadedActionIs(String episode, String action) {
        for (JSONObject uploaded : server.uploadedEpisodeActions) {
            if (episode.equals(uploaded.optString("episode")) && action.equals(uploaded.optString("action"))) {
                return true;
            }
        }
        return false;
